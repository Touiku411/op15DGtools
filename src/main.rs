use std::{fs, io::{self, Write}, path::Path, process::Command, thread::{self}, time::Duration};
...
fn enter_choice() -> i32{
        println!("       ---------ONEPLUS 15 升降級工具 (Support Windows&Linux)---------");
        println!("\x1b[1;31m====================================================");
...
fn clear_ota(){
        ...
        if dir.exists() {
                ...
                if input.trim().eq_ignore_ascii_case("y") {
                        ...
                        pause_terminal("已取消刪除,按 Enter 鍵繼續...");
                }
        }
        else{
                let _ = fs::create_dir_all(dir);
...
fn jiuzhuan(){
        let pre_fwpartitions = [
                "vbmeta", "vbmeta_system", "vbmeta_vendor", "vendor_boot", "init_boot",
                "boot", "recovery", "modem", "bluetooth", "cpucp", "cpucp_dtb",
                "dsp", "dtbo", "engineering_cdt", "featenabler", "oplus_sec",
                "shrm", "splash", "uefi"
        ];
        let cow_partitions = [
                 "system", "system_dlkm", "system_ext", "vendor", "product", "odm",
                "my_bigball", "my_carrier", "my_engineering", "my_heytap", "my_manifest",
                "my_product", "my_region", "my_stock", "odm_dlkm", "vendor_dlkm"
        ];
        let postfw_partitions = [
                "aop", "aop_config", "devcfg", "hyp", "imagefv", "keymaster",
                "oplusstanvbk", "qupfw", "tz", "uefisecapp", "abl", "xbl",
        ...
        for s in cow_partitions{
                for slot in ["a","b"]{
                        let partition_name = format!("{}_{}-cow",s,slot);
                        let _ = Command::new("fastboot")
                        .args(["delete-logical-partition", &partition_name])
                        .status();
                }
        }
        for s in postfw_partitions {
                let img = format!("{}.img",s);     
                let img_path = img_dir.join(&img);
                if img_path.exists() {
                        for slot in ["a","b"] {
